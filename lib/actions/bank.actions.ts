...
export const getAccounts = async ({ userId }: getAccountsProps) => {
  try {
    ...
    if (!banks || banks.length === 0) {
      console.log("DEBUG getAccounts() -> no banks for userId:", userId);
    ...
    const accounts = await Promise.all(
      banks.map(async (bank: any) => {
        if (!bank?.accessToken) {
          console.warn("getAccounts() -> bank missing accessToken:", bank);
          return null;
        }
        ...
        if (!accountData) {
          console.warn("getAccounts() -> no account data from Plaid for bank:", bank.$id);
          return null;
        }
...
export const getAccount = async ({ appwriteItemId }: getAccountProps) => {
  try {
    if (!appwriteItemId) {
      console.error("❌ getAccount() received EMPTY appwriteItemId");
      return null;
    }
    ...
    if (!bank) {
      console.error("❌ getBank() returned null — item does not exist");
      return null;
    }

    if (!bank.accessToken) {
      console.error("❌ getAccount() bank has no accessToken:", bank);
      return null;
    }
    ...
    if (!accountData) {
      console.error("❌ getAccount() Plaid returned no accountData for bank:", bank.$id);
      return null;
    }

    // get transfer transactions from appwrite (safe)
